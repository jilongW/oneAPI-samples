...
template <typename T>
static
bool test(queue &Q, int M, int N, int K, int comparision)
{
    std::cout << "\nBenchmarking (" << M << " x " << K << ") x (" << K << " x " << N << ") matrix multiplication, " << type_string<T>() << "\n";;
     std::cout << "do comparision times: " << comparision <<"\n";
    ...
    if ( comparision <= 1){
        ...
        std::vector<T> time_vec;
        time_vec.push_back(flops);
        for (int i = 1; i < comparision ; i++){
            time = time_gemms(ncalls + 1) - tare;
            avg = time / ncalls;
            flops = op_count / avg;
            flops*= 1e-9;
            if (flops >= 1000.) {
        ...
        std::cout << "\n" << comparision << " time's Average performance:\n";
        std::cout << "[";
